...
import { useGetCarByIdQuery, useGetUnavailableDatesQuery } from '../services/carApi';
import { useCreateRentalMutation } from '../services/rentalApi';
import { useCreateRentalOrderMutation, useVerifyRentalPaymentMutation } from '../services/paymentApi';
import { useAppSelector } from '../hooks/redux';
import Navbar from '../components/layout/Navbar';
import StripePayment from '../components/payment/StripePayment';
import { Calendar, AlertCircle, CheckCircle, ArrowLeft, XCircle, CreditCard } from 'lucide-react';
...
const RentalBooking = () => {
  ...
  const [createRentalOrder, { isLoading: isCreatingOrder }] = useCreateRentalOrderMutation();
  ...
  const getMinDate = () => {
    ...
    // If car has next available date and it's in the future, use that
    if (car?.availability?.nextAvailableDate) {
      ...
      return nextAvailable > today ? nextAvailable.toISOString().split('T')[0] : today.toISOString().split('T')[0];
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isDateRangeAvailable(startDate, endDate)) {
      setError('Selected date range includes booked dates. Please select different dates.');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        setError(orderErr?.data?.error || 'Failed to create payment order. Booking created but payment failed.');
  ...
  if (!car || !car.isForRent) {
    return (
      <div className="min-h-screen bg-light-subtle">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center">
            <p className="text-xl text-dark-600 mb-4">Car not found or not available for rent</p>
            <button
              onClick={() => navigate(-1)}
        ...
        <div className="glass rounded-xl p-6 lg:p-8">
          ...
          {!user?.isAadhaarVerified && (
            <div className="bg-warning-50 border border-warning-200 rounded-lg p-4 mb-6 flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-warning-900">Aadhaar Verification Required</p>
                <p className="text-sm text-warning-700 mt-1">
                  Please verify your Aadhaar document before booking a rental car. Contact admin for verification.
                </p>
              </div>
            </div>
          )}
          ...
          {success && (
            <div className="bg-success-50 border border-success-200 rounded-lg p-4 mb-6 flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-success-900">Booking Created Successfully!</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (endDate && selectedDate > endDate) {
                      setEndDate('');
                    } else if (endDate && !isDateRangeAvailable(selectedDate, endDate)) {
                      setEndDate('');
                      setError('Selected date range includes booked dates. Please select different dates.');
                    }
                  }}
                  ...
                  disabled={unavailableDates.length > 0 && unavailableDates.some(d => d === getMinDate())}
                />
                {car?.availability?.nextAvailableDate && !startDate && (
                ...
                {unavailableDates.length > 0 && (
                  <p className="text-xs text-dark-500 mt-1">
                    {unavailableDates.length} booked date{unavailableDates.length > 1 ? 's' : ''} excluded from selection
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (startDate && !isDateRangeAvailable(startDate, selectedDate)) {
                      setError('Selected date range includes booked dates. Please select different dates.');
                      return;
                    }
                ...
                />
                {startDate && !endDate && car?.availability?.bookedDates.length > 0 && (
            ...
            {startDate && endDate && (
              <div className="bg-dark-50 rounded-lg p-6">
                <h3 className="font-semibold text-dark-900 mb-4">Booking Summary</h3>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-dark-900">₹{car.rentalPrice?.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-dark-600">Total Days:</span>
                    <span className="font-semibold text-dark-900">{calculateDays()} days</span>
                  </div>
            ...
            {!paymentOrder ? (
              <div className="flex gap-4">
                ...
                <button
                  type="submit"
                  disabled={isCreating || !startDate || !endDate || !user?.isAadhaarVerified}
                ...
                >
                  {isCreating || isCreatingOrder ? 'Processing...' : 'Confirm Booking'}
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="bg-primary-50 border border-primary-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-primary-900">Booking Created Successfully!</p>
                  ...
                  <StripePayment
                    ...
                    onSuccess={async (paymentIntentId) => {
                      ...
                      try {
                        ...
                        setError(err?.data?.error || err?.data?.details || 'Payment verification failed');
