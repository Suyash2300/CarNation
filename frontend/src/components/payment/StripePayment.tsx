  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (validationResult.error) {
        setError(
          validationResult.error.message || "Please check your card details"
        );
        setIsLoading(false);
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        setError(
          "Payment requires additional authentication. Please try again or use a different card."
        );
        setIsLoading(false);
        setIsProcessing(false);
        ...
        setTimeout(async () => {
          try {
            const {
              ...
              error: retrieveError,
            } = await stripe.retrievePaymentIntent(paymentIntent.id);
            if (retrieveError) {
              setError(
                retrieveError.message ||
                  "Payment is processing. Please check your dashboard in a moment."
              );
              ...
              setError(
                ...
                }. Please refresh and check your dashboard.`
              );
            }
          } catch (err) {
            setError(
        ...
        setError(
          `Payment status: ${paymentIntent.status}. Please contact support if the issue persists.`
        );
        setIsLoading(false);
        setIsProcessing(false);
      }
      ...
      <div className="space-y-4">
        <div className="bg-warning-50 border border-warning-200 rounded-lg p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-semibold text-warning-900 text-sm mb-1">
              MOCK PAYMENT MODE
            </p>
            <p className="text-xs text-warning-700 mb-2">
              Stripe test keys not configured. Payment will be simulated
              locally.
            </p>
            <p className="text-xs text-warning-600 font-medium">
              💡 To see the real Stripe payment interface, add your Stripe test
              keys to backend/.env:
            </p>
