...
const ChatWindow = ({ 
  ...
  const groupedMessages = useMemo(() => {
    ...
    const grouped: Array<{
      date: string;
      messages: Array<Message & { showAvatar: boolean; showDateSeparator: boolean; isFirstInGroup: boolean }>;
    }> = [];
  ...
  useEffect(() => {
    const socketInstance = socketTokenOverride ? getSocketWithToken(socketTokenOverride) : getSocket();
    if (socketInstance) {
      ...
      socketInstance.on('user-typing', (data: { userId: string; conversationId: string; userName?: string }) => {
        if (data.conversationId === conversation.id && data.userId !== effectiveUserId) {
          setIsTyping(true);
  ...
  const otherParticipant = conversation.participant1Id === effectiveUserId
    ...
    <div className="flex flex-col h-full bg-white dark:bg-dark-900">
      ...
      <div className="bg-gradient-to-r from-primary-50 to-primary-100 dark:from-dark-800 dark:to-dark-700 border-b border-dark-200 dark:border-dark-700 p-3 sm:p-4 flex items-center gap-2 sm:gap-3 shadow-sm min-w-0">
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <h3 className="font-semibold text-dark-900 dark:text-white text-sm sm:text-base md:text-lg break-words leading-tight">{otherParticipant.name}</h3>
          {isTyping ? (
            <p className="text-xs sm:text-sm text-primary-600 dark:text-primary-400 flex items-center gap-1 flex-wrap">
              ...
              <span className="ml-1 break-words">{typingUser} is typing...</span>
            </p>
          ) : (
