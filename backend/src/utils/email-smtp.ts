...
/**
 * Send password reset email using Brevo SMTP directly
 * This works without needing API activation
 */
export const sendPasswordResetEmailSMTP = async (
  email: string,
  resetUrl: string
): Promise<void> => {
  ...
  const smtpUser = process.env.BREVO_SMTP_USER || process.env.BREVO_SMTP_EMAIL; // Your Brevo login email
  const smtpPass = process.env.BREVO_SMTP_PASSWORD; // SMTP password (different from API key)
  
  const fromEmail = process.env.BREVO_FROM_EMAIL || 'CarNation <carnation71212@gmail.com>';
  const fromName = process.env.BREVO_FROM_NAME || 'CarNation';
